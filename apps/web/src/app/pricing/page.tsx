...
export default function PricingPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      {access ? (
        <div className="panel p-4 text-sm text-ink-300">
          Current plan:{' '}
          <span className="font-medium capitalize text-ink-50">{access.plan}</span>
          ...
          <Link href="/billing" className="text-accent-soft hover:underline">
            Manage billing
          </Link>
        </div>
      ) : null}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {plans.map((plan) => (
          ...
          >
            ...
            <div className="mt-4 font-mono text-3xl text-accent-soft">
              {plan.priceLabel}
              <span className="text-base text-ink-400">{plan.intervalLabel}</span>
            </div>
      ...
      <section className="panel space-y-2 p-5 text-sm text-ink-400">
        <h3 className="font-display text-base font-semibold text-ink-100">Stripe setup</h3>
        <p>
          Create a Product + recurring Price in Stripe, then set{' '}
          <code className="text-ink-300">STRIPE_SECRET_KEY</code>,{' '}
          <code className="text-ink-300">STRIPE_PRICE_PRO_MONTHLY</code>, and{' '}
          <code className="text-ink-300">STRIPE_WEBHOOK_SECRET</code>. Webhook endpoint:{' '}
          <code className="text-ink-300">POST /api/billing/webhook</code>
        </p>
        <p>
